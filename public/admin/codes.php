...
if(
 ...
 if(isset($_GET['g']) && $_GET['g'] == 'new') {
  $id = $tigers->cleanMys((int)$_GET['d']) + 1;
?>
   ...
   while($getCat = $scorpions->obj($true)) {
    ...
    while($items = $scorpions->obj($pull)) {
	   echo "  <option value=\"" . $items->catID . "\"";
		 if(in_array($getitems->catID, explode('!', $getItem->cCategory))) {
	    echo " selected=\"selected\"";
	   }
...
?>
 <p class="tc"><img src="<?php echo $seahorses->getOption('codes_img_http') . $getItem->cFile; ?>" alt=""></p>
<?php 
 } 
?>
...
<tbody><tr>
 ...
 <td class="tc"><img src="<?php echo $seahorses->getOption('codes_img_http') . $getItem->cFile; ?>" alt=""></td>
...
<?php 
 ...
 if($count > 0) {
  $codesNumb = $cheetahs->codeCount('n', 'all');
