...
<fieldset> 
 ...
 <p><label for="joined"><strong>Category:</strong></label> <select name="joined" id="joined" class="input1">
<?php 
 $select = "SELECT * FROM `$_ST[categories]` WHERE `parent` = '0' ORDER BY `catname` ASC";
 $true = $scorpions->query($select);
 if($true == false) {
...
?>
 <p class="tc"><img src="<?php echo $seahorses->getOption('jnd_http') . $getItem->jImage; ?>" alt=""></p>
<?php 
 } 
?>
 ...
 <p><label><strnog>New Image:</strong></label> 
 <input name="image" class="input1" type="file"></p>
 <p><label><strong>Made By You?</strong></label> 
<?php 
if($getItem->jMade == 'y') {
...
<fieldset>
 <legend>Categories</legend>
 <p class="tc"><label>Categories:</label> <select name="category[]" class="input1" multiple="multiple" size="7">
<?php 
 $select = "SELECT * FROM `$_ST[categories]` WHERE `parent` = '0' ORDER BY `catname` ASC";
 $true = $scorpions->query($select);
 if($true == false) { 
