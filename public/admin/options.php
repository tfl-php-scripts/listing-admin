...
if(isset($_POST['action']) && $_POST['action'] == 'Edit Options') {
 ...
 if($opt == '1') {
  ...
  $my_emailNow = $tigers->cleanMys($_POST['my_email']);
  if(empty($my_emailNow)) {
   $tigers->displayError('Form Error', 'Your <samp>e-mail address</samp>' . 
	...
	elseif(!preg_match("/([A-Za-z0-9-_\.]+)@(([A-Za-z0-9-_]+\.)+)([a-zA-Z]{2,4})$/i", $my_emailNow)) {
  ...
  $my_url = strtolower($tigers->cleanMys($_POST['my_website']));
  if(empty($my_url)) {
   $tigers->displayError('Form Error', 'Your <samp>website</samp>' . 
	 ' is empty.', false);
  } elseif ($my_url == 'http://' || strstr($my_url, 'http://') === false) {
  ...
  if(($my_password_valid1 !== $my_password_valid2)) {
   $tigers->displayError('Form Error', 'If you\'re changing your' . 
	...
	if(!empty($my_password_valid1)) {
	 if(strlen($my_password_valid1) < 8) {
	  $tigers->displayError('Form Error', 'If you\'re changing your' . 
 ...
 elseif ($opt == '4') {
  ...
  if(empty($markupopt) || !in_array($markupopt, array_keys($get_markup_array))) {
...
<fieldset>
 ...
 SPAM words you <ins>can't</ins> delete, and are there to give an idea of what
 to add. You are free to add, edit and delete any other SPAM words.</p>
...
<fieldset>
 ...
 value="<?php echo $leopards->getPaths('url', $seahorses->getOption('jnd_http'), '', 'images/joined/'); ?>"></p>
</fieldset>
...
<fieldset>
 ...
 value="<?php echo $leopards->getPaths('url', $seahorses->getOption('wsh_http'), '', 'images/wishlist/'); ?>"></p>
 <p class="clearBottom"></p>
...
?>
 ...
 This should <ins>only</ins> be used if we're importing a .txt file from 
...
<?php if($seahorses->getOption('notify_approval') == 'y') echo " checked=\"checked\""; ?> type="radio" value="y"> On
 <input name="approval" class="input3"
<?php if($seahorses->getOption('notify_approval') == 'n') echo " checked=\"checked\""; ?> type="radio" value="n"> Off
 </p>
</fieldset>
...
<fieldset>
 ...
 <input name="per_members" class="input1" type="text" value="<?php echo $seahorses->getOption('per_members'); ?>"></p>
 ...
 <input name="per_page" class="input1" type="text" value="<?php echo $seahorses->getOption('per_page'); ?>"></p>
</fieldset>
...
<fieldset>
 ...
 value="<?php echo $leopards->getPaths('url', $seahorses->getOption('adm_http'), '', ''); ?>"></p>
 <p style="clear: both; margin: 0 0 2% 0;"></p>
 ...
 value="<?php echo $leopards->getPaths('url', $seahorses->getOption('aff_http'), '', 'images/affiliates/'); ?>"></p>
 <p style="clear: both; margin: 0 0 2% 0;"></p>
 ...
 value="<?php echo $leopards->getPaths('url', $seahorses->getOption('img_http'), '', 'images/'); ?>"></p>
</fieldset>
...
?>
 ...
 type="text" value="<?php echo $seahorses->getOption('akismet_key'); ?>"></p>
</fieldset>
...
<fieldset>
 ...
 <p><label><strong>Password Hint:</strong></label> 
  <input name="mypasshint" class="input1" type="text" value="<?php echo $seahorses->getOption('user_passhint'); ?>">
 </p>
</fieldset>
