...
<?php 
 ...
 elseif (isset($_POST['action']) && $_POST['action'] == 'Add Member') {
  ...
  if(empty($email)) { 
   $tigers->displayError('Form Error', 'The <samp>e-mail</samp> field' . 
	 ' is empty.', false);
  ...
  if(!empty($url)) {
   if(!strstr($url, 'http://')) {
    $tigers->displayError('Form Error', 'The <samp>URI</samp> does not' . 
...
<?php 
 ...
 elseif (isset($_POST['action']) && $_POST['action'] == 'Edit Member') {
  ...
  $email = $tigers->cleanMys($_POST['email']);
  if(empty($email)) { 
   $tigers->displayError('Form Error', 'The <samp>email</samp> field is empty.', false);
  } elseif (!preg_match("/([A-Za-z0-9-_\.]+)@(([A-Za-z0-9-_]+\.)+)([a-zA-Z]{2,4})$/i", $email)) {
  ...
  if(!empty($url)) { 
   if($url == 'http://' || strstr($url, 'http://') === false) {
    $tigers->displayError('Form Error', 'The <samp>site URL</samp> field is not' . 
...
<?php 
   while($getItem = $scorpions->obj($true)) {
		$qw = !isset($_GET['listingid']) ? "" : "&#38;listingid=" . $tigers;
