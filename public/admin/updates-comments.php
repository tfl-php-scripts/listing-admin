...
<fieldset>
 ...
 <input name="email" class="input1" type="text" value="<?php echo $getItem->cEmail; ?>"></p>
 <p><label><strong>URL:</strong></label> 
 <input name="url" class="input1" type="text" value="<?php echo $getItem->cURL; ?>"></p>
...
elseif (isset($_POST['action']) && $_POST['action'] == 'Edit Comment') {
 ...
 if(!empty($url)) {
  if($url == 'http://' || strstr($url, 'http://') == false) {
   $tigers->displayError('Form Error', 'The <samp>site URL</samp> field is' . 
 ...
 $update = "UPDATE `$_ST[updates_comments]` SET `eNiq` = '$entry_id', `cName`" . 
...
<?php
  while($getItem = mysql_fetch_array($true)) {
   ...
   ' class="pending_spam"' : '';
?>
<tbody<?php echo $colour; ?>><tr>
 <td class="tc"><input name="comment[]" type="checkbox" value="<?php echo $getItem->cID; ?>"></td>
 <td class="tc">
<?php 
