...
if(
 isset($_GET['sec']) && 
 ($_GET['sec'] == 'install' ||in_array($_GET['sec'], array_keys($get_addon_array)))
) {
...
?>
 ...
 <input name="codes_order_input" class="input1" type="text" value="<?php echo $seahorses->getOption('codes_order'); ?>"></p>
</fieldset>
...
<fieldset>
 ...
 <input name="codes_formurl_input" class="input1" type="text" value="<?php echo $seahorses->getOption('codes_formurl'); ?>"></p>
 <p class="clearBottom"></p>
 ...
 <a href="http://www.matisse.net/bitcalc/" title="External Link: matisse.net">at this website &#187;</a>:
 </label>
 <input name="codes_filesize_input" class="input1" type="text" value="<?php echo $seahorses->getOption('codes_filesize'); ?>"></p>
</fieldset>
...
?>
 ...
 <input name="updates_url_opt" class="input1" type="text" value="<?php echo $seahorses->getOption('updates_url'); ?>"></p>
 <p style="clear: both; margin: 0 0 2% 0;"></p>
...
<fieldset>
 ...
 <p class="noteButton">The following plugins are for comments <ins>only</ins>. To 
 set each plugin, choose "On" or "Off" from each drop down menu.</p> 
...
elseif (
 ...
 if(empty($secpost) || !in_array($secpost, array_keys($get_addon_array))) {
 ...
 if($secpost == 'codes' && $sechash == sha1($get_addon_array['codes'])) {
  ...
  if(in_array($codes_opt_input, $ynArray)) {
   if($seahorses->getOption('codes_opt') != $codes_opt_input) {
 ...
 elseif ($secpost == 'install' && $sechash == sha1('Install')) {
  ...
  if(
   $installaddon != 'none' &&
   !in_array($installaddon, array_keys($get_addon_array))
  ) {
   $tigers->displayError('Form Error', 'The addon you want to install is not' . 
   ' valid, m\'dear!', false);
  } 
  if($uninstalladdon != 'none') {
   if(!in_array($uninstalladdon, array_keys($get_addon_array))) {
   ...
   * Install the addon! 
   */ 
  if($installaddon != 'none') {
   if($cheetahs->isInstalled($installaddon) == false) {
   ...
   */ 
  if($uninstalladdon != 'none') {
   if($cheetahs->isInstalled($uninstalladdon) == true) {
 ...
 elseif ($secpost == 'kim' && $sechash == sha1($get_addon_array['kim'])) {
  ...
  if(in_array($kim_opt_input, $ynArray)) {
   if($seahorses->getOption('kim_opt') != $kim_opt_input) {
 ...
 elseif ($secpost == 'lyrics' && $sechash == sha1($get_addon_array['lyrics'])) {
  ...
  if(in_array($lyrics_opt_input, $ynArray)) {
   if($seahorses->getOption('lyrics_opt') != $lyrics_opt_input) {
 ...
 elseif ($secpost == 'quotes' && $sechash == sha1($get_addon_array['quotes'])) {
  ...
  if(in_array($quotes_opt_input, $ynArray)) {
   if($seahorses->getOption('quotes_opt') != $quotes_opt_input) {
