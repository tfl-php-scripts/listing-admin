...
if(isset($_POST['action']) && $_POST['action'] == 'Update Information') {
 ...
 if(empty($name)) { 
  $tigers->displayError('Form Error', 'You have not filled out the <samp>name' . 
  '</samp> field.', false);
 } elseif (strlen($name) > 20) {
  $tigers->displayError('Form Error', 'Your <samp>name</samp> is too long. Go' . 
  ' back and shorten it.</p>', false);
 }
 $email = $tigers->cleanMys($_POST['email']);
 $new_email = $tigers->cleanMys($_POST['new_email']);
 if(empty($email)) {
 ...
 if(!empty($new_email)) {
  if(
   !preg_match("/([A-Za-z0-9-_\.]+)@(([A-Za-z0-9-_]+\.)+)([a-zA-Z]{2,4})$/i", $new_email)
  ) {
   $tigers->displayError('Form Error', 'The characters specified in the <samp>' . 
   'new email</samp> field are not allowed.', false); 
  }
 } 
 $new_url = $tigers->cleanMys($_POST['new_url']);
 if(!empty($new_url)) {
  if($new_url == 'http://' || strstr($new_url, 'http://') == false) {
 ...
 if($snakes->checkMember($name, $email, $password, $id) == false) {
  $octopus->writeError(
  ...
  $tigers->displayError('Script Error', 'It appears the script has not' . 
	...
	' correct e-mail address/name, or you have entered a incorrect password.' . 
 ...
 if(isset($_POST['new_country']) && !empty($new_country) && $_POST['country'] == 'Choose') {
...
<fieldset>
 ...
 <input name="new_email" class="input1" type="text"<?php echo $mark; ?>></p>
 <p><label><strong>New URL:</strong></label> 
 <input name="new_url" class="input1" type="text"<?php echo $mark; ?>></p>
...
<fieldset>
 ...
 <p><label><strong>Show E-mail:</strong></label> 
  <input name="visible" checked="checked" class="input3" type="radio" value="2"<?php echo $mark; ?>> Leave
