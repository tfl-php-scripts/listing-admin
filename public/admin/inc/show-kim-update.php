...
if(isset($_POST['action']) && $_POST['action'] == 'Update Information') {
 ...
 if(!empty($new_url)) { 
  if(
	 $new_url == 'http://' || 
	 preg_match("@(http://)([A-Za-z0-9-_\./?]+)([A-Za-z\.]{2,4})/?$@i", $new_url) === false
  ) {
 ...
 $update .= " `mPending` = '1', `mUpdate` = 'y' WHERE `mEmail` = '$email' AND" . 
...
<legend>Password</legend>
 ...
 <input name="passwordo" class="input1" type="password"<?php echo $mark; ?>></p>
...
<fieldset>
 ...
 <input name="visible" checked="checked" class="input3" type="radio" value="3"<?php echo $mark; ?>> Leave
