  ...
  public function affiliatesList($i = '', $o = '', $b = 'id') {
   ...
   return $all;
	...
	/** 
	 ...
	 $p = Current page we're on (for pagination) 
	*/ 
	# ----------------------------------------------------------- 
	public function sortAffiliates($i, $s = '', $a = array(), $p) {
	 global $_ST, $scorpions, $get_affsearch_array, $laantispam, $per_page, 
   $snakes, $wolves;
  ...
  public function countAffiliates($i) {
   ...
   if($i != '0') {
    ...
    if($true->status == false) {
     $listingadmin->displayError('Database Error', 'The script was nable to' . 
