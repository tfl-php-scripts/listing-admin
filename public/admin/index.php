...
<ins>DO NOT DISAPPOINT AND POSSIBLY DAMAGE YOUR SERVER</ins>.</p>
<?php 
}
...
<?php 
 $con = 1;
 $exp = explode('Listing Admin ', $laoptions->version);
 $url = "http://scripts.wyngs.net/versions/";

 require_once('inc/fetch/rss_fetch.inc');
 $items = fetch_rss($url);
 foreach($items->items as $item) { 
  if($con > 0) {
   if($item['title'] === 'Listing Admin' && $item['description'] !== $exp[1]) {
	  echo "<p class=\"errorButton\"><span class=\"error\">Error!</span> It appear" . 
    "s you do not have the latest version of Listing Admin! Please visit the" . 
	  " <a href=\"http://scripts.wyngs.net/downloads/\" title=\"External Link:" . 
    " Pumpin'!\">downloads page at Pumpin'! &#187;</a> to download the latest" . 
    " version.</p>\n";
	 }
  }
  $con--;
...
<div id="statistics">
 ...
 <table>
  <tbody><tr>
   ...
   <strong><?php echo $pending; ?></strong> Pending)</td>
  </tr></tbody>
 </table>
 </section>
 <section id="stLeft">
